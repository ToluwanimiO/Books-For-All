<template>
  ...
  <div class="mt-4 pt-4">
    ...
    <div v-else>
      <el-table class="w-100" :data="pageData" @selection-change="handleSelectionChange">
        <el-table-column type="selection" width="55" />

        <!-- Title Column -->
        <el-table-column width="250">
          <template #header>
            <eden-table-column-header :label="'School Name'" :property="'name'" :sort-property="sort.property"
        ...
        <el-table-column>
          <template #header>
            <eden-table-column-header :label="'Location'" :property="'location'" :sort-property="sort.property"
              @sort="sortPageData($event, 'author')" />
          </template>
          <template v-slot="scope">
            <div>{{ scope.row.location.address + ', ' + scope.row.location.state + ', ' + scope.row.location.country }}
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column width="200">
          <template #header>
            <eden-table-column-header :label="'Library Limit'" :property="'libraryLimit'" :sort-property="sort.property"
        ...
        <el-table-column width="200" v-if="user.role === 'tolu'">
          <template v-slot="scope">
            <div class="text-success text-cursor" ><router-link :to="{name:'donate'}">Donate</router-link></div>
          </template>
        </el-table-column>
      </el-table>
...
const getSchools = () => {
  ...
  getAllSchools()
    ...
    .catch((err) => {
      ...
      ElMessage.error({ message: err.response?.data?.message || "Error getting list of schools" });
      loading.value = false;
    });
};
